    ...
    for space in model.spaces: 
        ...
        for i, point in enumerate(space.get_shape().get_points(wrt=space.get_shape().get_frame())):
            position_shape_point_to_space_frame = {
                "@id" : "coord:coord-position-point-{i}-to-{name}-frame".format(name=space.name, i=i),
                "@type" : ["PositionReference", "PositionCoordinate","VectorXY"],
                "of-position" : "geor:position-point-{i}-to-{name}-frame".format(name=space.name, i=i),
        ...
        for i, wall in enumerate(space.walls):
            ...
            pose_wall_frame_to_space_frame = {
                "@id" : "coord:coord-pose-of-wall-{i}-frame-to-{name}-frame".format(name=space.name, i=i),
                "@type" : ["PoseReference", "PoseCoordinate","VectorXY"],
                "of-pose" : "geor:pose-of-wall-{i}-frame-to-{name}-frame".format(name=space.name, i=i),
                "as-seen-by" : "geom:frame-center-{name}".format(name=space.name),
                "unit": ["M","degrees"],
                "theta" : theta,
                "x": origin[0],
            ...
            for j, point in enumerate(wall.polygon):
                ...
                position_shape_point_to_wall_frame = {
                    "@id" : "coord:coord-position-corner-{j}-to-{name}-wall-{i}-frame".format(i=i, name=space.name, j=j),
                    "@type" : ["PositionReference", "PositionCoordinate","VectorXY"],
                    "of-position" : "geor:position-corner-{j}-to-{name}-wall-{i}-frame".format(i=i, name=space.name, j=j),
                    "with-respect-to": "geom:point-frame-{name}-wall-{number}".format(name=space.name, number=i),
                    "as-seen-by" : "geom:frame-{name}-wall-{i}".format(name=space.name, i=i),
                    "unit": "M",
                    "x": point[0],
                    "y": point[1]
                }
